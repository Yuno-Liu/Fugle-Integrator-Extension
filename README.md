...
-   **關鍵財務指標儀表板**：
    ...
    -   殖利率、毛利率、ROE、ROA
    -   市值計算 (自動換算為億或兆)
    -   **市場排名指標**：自動比較個股與全市場指標，顯示該股票在全市場中的排名百分比位置
-   **主力買賣超分析**：
...
-   **即時時間顯示**：
    ...
    -   可懸停查看完整日期與星期
-   **便捷分析工具列**：
    -   **全方位搜尋**：支援搜尋股票代號、名稱、概念股、產業與集團，快速查找相關個股並直接跳轉。
    -   **相關股票推薦**：搜尋時自動推薦同分類（同概念、同產業、同集團）的相關股票。
...
-   **UI 優化與自定義**：
    ...
    -   **獨立視窗**：支援將資訊卡片彈出至獨立視窗，方便多螢幕使用者。資訊卡片在獨立視窗中保留所有互動功能，區塊展開/折疊狀態自動同步。
    -   **顯示開關**：可隨時隱藏或顯示資訊卡片。
    -   **分組摺疊**：各資訊區塊可獨立展開/摺疊，狀態自動保存至瀏覽器 localStorage。
...
💡 **提示**：可同時安裝多個擴充功能，它們彼此獨立且相輔相成。

### 實用工具：網頁自動刷新器 (AutoRefresh-TS)

本專案提供一個獨立的網頁自動重新整理擴充功能，主要功能為：
- **自訂秒數刷新**：支援任意秒數自訂輸入與 `+1`、`+5`、`-1`、`-5` 步進調整，並提供 5s、10s、30s、1m、5m、10m 常用預設膠囊。
- **分頁獨立控制**：每個分頁具備完全獨立的計時與刷新設定，互不干擾。
- **即時倒數與動態進度條**：彈跳視窗內建平滑進度條與大字體倒數顯示。
- **網頁懸浮倒數小工具 (Floating Widget)**：在網頁角落顯示極簡毛玻璃倒數膠囊，支援自由拖曳與一鍵暫停。
- **擴充套件圖示 Badge 即時秒數**：無需開啟彈跳視窗，隨時於瀏覽器工具列掌握倒數狀態。
- **進階防快取與隨機浮動**：支援強制重新整理（略過瀏覽器 Cache）與隨機浮動秒數 (Jitter，降低被伺服器偵測風險)。

**安裝步驟：**
使用「載入未封裝項目」選擇 `ChromeExtension/AutoRefresh-TS/dist` 資料夾即可完成安裝。
...
4.  **工具按鈕**：
    -   點擊 `🔍 搜尋` 可開啟搜尋視窗，輸入關鍵字（如 "AI"、"2330"、"長榮"）即可快速查找個股或分類，並自動推薦同分類的相關股票。
    -   點擊 `📊 預估量` 可查看根據盤中時間計算的當日預估成交量。
    -   點擊分析網站按鈕 (`📈 WantGoo`、`💬 CMoney`、`📊 TV`、`🏛️ 法人`、`👤 主力`、`🤖 Gemini`) 可開新分頁查詢。
    -   點擊位置按鈕 (如 `➡️ 靠右`) 可切換卡片顯示位置。
    -   點擊 `❐ 彈出` 可將卡片開啟於獨立視窗（支援多螢幕使用）。
    -   點擊 `🔑 Token` 可設定成交量 API Token。
    -   使用 `資訊卡` 滑動開關可暫時隱藏或顯示資訊卡片。
